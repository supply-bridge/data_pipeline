...
当我们要决定是否从索引中检索时，[检索代理](https://python.langchain.com/docs/tutorials/qa_chat_history/#agents)就派上用场了。

要实现检索代理，我们只需让 LLM 访问检索工具即可。
...
class AgentState(TypedDict):
    # The add_messages 函数定义了更新的处理方式
    # 默认是替换. add_messages 参数表示“添加"
...
* 从代理开始，调用模型
* 代理做出调用函数的决定
* 如果是，则采取行动调用工具（retriever）
* 然后调用代理，并将工具输出添加到信息中（state)
